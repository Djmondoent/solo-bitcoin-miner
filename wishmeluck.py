...
RPC_URL = os.environ.get("RPC_URL", "http://veevme.ddns.net:8332")
RPC_USER = os.environ.get("RPC_USER", "admin")
RPC_PASS = os.environ.get("RPC_PASS", "Veev1Roll")
...
def tx_make_coinbase_p2sh(coinbase_script, address, value, height):
    """
    Create a coinbase transaction.
    ...
    # Create a pubkey script
    # OP_HASH160 <len to push> <pubkey> OP_EQUAL
    pubkey_script = "a9" + "14" + bitcoinaddress2hash160(address) + "87"
...
def block_mine(block_template, coinbase_message, extranonce_start, address, timeout=None, debugnonce_start=2315460000):
    """
    Mine a block.
    ...
    while True:
        ...
        block_template['merkleroot'] = coinbase_tx['hash']
    ...
    while True:
        try:
          block_template = rpc_getblocktemplate()
           ...
           print("Network down - retrying 5 secs later...")
           time.sleep(5)

        print("Mining block template, height {:d}...".format(block_template['height']))
        mined_block, hash_rate = block_mine(block_template, coinbase_message, 0xca110000, address, timeout=60)
